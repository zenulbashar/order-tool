...
| F7 | **Partial** — harness shipped, RLS deliberately deferred | F7's complaint was that *"nothing — not the type system, not a test, not the database — will catch"* a query missing its tenant filter. A test now does: `test/tenant-scoping.test.ts` derives the 39 venue-scoped tables from the schema, walks every query in `app/` and `lib/`, and fails CI on any statement without a tenant predicate; deliberate cross-venue reads sit in an exemption list where **every entry carries a reason**, so they are enumerated rather than implicit. RLS itself is **deliberately NOT implemented**, per §8.1's own instruction — the counter-sources (PlanetScale, Neon, AWS Prescriptive Guidance) are still unread, and the naive policy on this exact `venue_members` pattern is benchmarked as *timing out*. Decision and re-entry criteria: `docs/audit/TenantIsolation.md`. |
...
- **Shipped 2026-08-01** (eighth remediation pass): all four items. Adding axe
  was not a formality — it failed immediately on four real contrast defects
  (see `docs/audit/Accessibility.md` for the before/after table) and on a
  landing page with no `<main>` landmark. The contrast maths now lives in one
  place (`lib/contrast.ts`), shared by the publish gate and the branded
  emails, so the two cannot drift.
