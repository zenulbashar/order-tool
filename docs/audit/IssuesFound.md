...
| D4 | Low | One-off buttons re-implement `buttonStyles(...)`; hand-rolled segmented controls and page headers bypass `<Segmented>`/`<PageHeader>`; arbitrary radii (`rounded-[14px]` etc.) where tokens exist. | ◑ Partial — 3 of the 4 halves closed: **page headers** (the tables board was the last of 36; now pinned by a derived test), **segmented controls** (one genuine hand-roll, whose `aria-pressed` also mis-announced a single-choice group), and **radii** (a false lead bar one conversion). The **buttons** half is measured and open — see below. |
