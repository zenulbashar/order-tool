...
| D2 | Medium | **21** files copy-paste the `controlClass` recipe onto raw `<input>/<select>/<textarea>`. The copies had DRIFTED: `px-2.5` instead of `px-3`, and missing `placeholder:text-muted`, `read-only:bg-sand/40` and **both `disabled:` rules** — so those controls gave no feedback when disabled or read-only. | ◑ Partial — the disabled/read-only gap is FIXED (state-prefixed, so default rendering unchanged); the duplication and the padding divergence still need the visual review |
