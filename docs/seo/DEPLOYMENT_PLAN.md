...
## Phase 1 — Core Web Vitals + trust (1–2 days)

The video's "Lighthouse 100" chapter. Highest-leverage performance and credibility fixes.
...
## Phase 2 — The content engine (the video's tactic #1, ongoing)

Blog-at-scale, adapted to this repo's typed-content system (no CMS needed).
...
## Phase 3 — Programmatic landing pages (the video's tactic #2)

Money pages: service × audience/city, done tastefully (dozens, not thousands — the video's own warning).
...
## Phase 4 — Measurement loop + off-page (ongoing)
...
Extensions to the shipped feature, in likely order of value:

1. **Venue FAQs as first-class data:** a `venue_faqs` table + owner CRUD, so applied AEO FAQ suggestions render on the storefront **and** emit FAQPage JSON-LD (today they're copy-paste suggestions).
